...
export default function Home() {
	...
	useEffect(() => {
		connectWallet();
		getAllTasks();
	}, [correctNetwork, isLoggedin, currentAcct, textInput, tasks]);
	...
	const addTask = async (e) => {
		...
		try {
			...
			if (ethereum) {
				...
				ToDoTaskContract.addTask(task.taskText, task.isDeleted)
					.then((res) => {
	...
	const deleteTask = (key) => async () => {
		try {
			if (ethereum) {
				...
				const deleteTaskTnx = await ToDoTaskContract.deleteTask(
					key,
					true,
				);
	...
	return (
		<div className="bg-gradient-to-r from-cyan-500 to-blue-500 h-screen w-screen flex justify-center py-4">
			...
			{isLoggedin && !correctNetwork && <WrongNetworkMessage />}
			{!isLoggedin && (
